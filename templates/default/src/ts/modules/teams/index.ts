...
import { SessionExpiredError } from "../../infrastructure/http";
	...
	app.ports.requestTeamCatalog?.subscribe(async () => {
		try {
			...
			if (e instanceof SessionExpiredError) {
				app.ports.receiveAppError?.send({
					source: "teams",
					message: "Your Azure DevOps session has expired. Please reload the page to sign in again.",
				});
			} else {
